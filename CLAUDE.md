...
כשמתבקש להריץ את הפרויקט — תמיד הרץ עם:
```bash
./start.sh
```
זה מפעיל
...
ו-
`sw.js`
ומרענן את הדפדפן אוטומטית.

כל בקשה שכוללת אחת מהמילים:
"תריץ", "תראה לי", "תפתח", "רוץ", "הרץ", "run", "show me", "open"
— חייב להפעיל את `./start.sh` ולא שום דרך אחרת.

## 🚫 אסור להשתמש ב-preview_start

**לעולם אל תשתמש בכלי `preview_start`** — הוא מפעיל שרת אחר שמבטל את הרענון האוטומטי של `browser-sync`.
המשתמשת עוקבת אחרי השינויים בלייב דרך `./start.sh` בלבד.
מותר להשתמש ב-`preview_eval` ו-`preview_screenshot` לאימות בלבד — לא להפעלת שרת.
