...
כשמתבקש למזג ולדחוף, **תמיד** בצע את שני השלבים:

1. `git push origin master` — שמירת הקוד בגיטהאב
2. `firebase deploy --only hosting` — עדכון האתר החי

**לא לסיים** בלי ששני השלבים הצליחו. אם אחד נכשל — לדווח.
