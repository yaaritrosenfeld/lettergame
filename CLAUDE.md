...
פורמט: `DD.mm.yy.xx`  
- `DD` — יום, `mm` — חודש, `yy` — שנה (2 ספרות)
- `xx` — מספר עדכון באותו יום (01, 02, ...)

הקבוע `APP_VERSION` נמצא בשורה הראשונה של ה-`<script>` ב-`index.html`.

**בכל "מזג ודחוף":**
1. עדכן את `APP_VERSION` — אם אותו יום כמו הגרסה הקודמת, העלה את `xx` ב-1; אחרת אפס ל-`01` עם התאריך החדש.
2. עדכן את שם הקאש ב-`sw.js` — `olam-otiyot-vN` → הגדל את N ב-1. **חובה** — בלי זה המשתמשים יקבלו גרסה ישנה מהקאש.
3. **בסיום — דווח ללקוח מה מספר הגרסה העדכני.**

## 🚀 "מזג ודחוף" — סדר פעולות חובה

כשמתבקש למזג ולדחוף, **תמיד** בצע את שלושת השלבים:

1. עדכן `APP_VERSION` ב-`index.html` + שם קאש ב-`sw.js` (ראה למעלה)
2. `git push origin master` — שמירת הקוד בגיטהאב
3. `firebase deploy --only hosting` — עדכון האתר החי

**לא לסיים** בלי ששלושת השלבים הצליחו. אם אחד נכשל — לדווח.
